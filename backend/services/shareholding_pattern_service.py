...
def get_shareholding_pattern_file_id(company_number: int) -> str:
    ...
    drive_id = tl.get(ticker[0], f"default_shp_file_{company_number}")
    return drive_id

if __name__ == "__main__":
    # Example usage
    try:
        company_number7 = 12  # Replace with a valid company number
        file_id = get_shareholding_pattern_file_id(company_number7)
        print(f"Shareholding Pattern File ID for company number {company_number7}: {file_id}")
